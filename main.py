...
class MainWindow:
    ...
    def draw(self, particles):
        ...
        existing = particles.exploded[:] == 0
        if particles.positions[0].any() != 0:
            screen_array = pygame.surfarray.pixels2d(self.display)
            kolor_int = (particles.colors[:, 0].astype(int) << 16) + (particles.colors[:, 1].astype(int) << 8) + particles.colors[:, 2].astype(int)
            kolor_extra_int = (particles.extra_particles_colour[:, 0].astype(int) << 16) + (particles.extra_particles_colour[:, 1].astype(int) << 8) + particles.extra_particles_colour[:, 2].astype(int)

            x = np.clip(particles.positions[existing, 0].astype(int), 0, self.width - 201)
            y = np.clip(particles.positions[existing, 1].astype(int), 0, self.height - 1)

            xextra = np.clip(particles.extra_particlesx[~existing, :].astype(int), 0, self.width - 201).flatten() # x locations of all extra particles
            yextra = np.clip(particles.extra_particlesy[~existing, :].astype(int), 0, self.height - 1).flatten() # y locations of all extra particles

            color_repete = np.repeat(kolor_extra_int[~existing], 20, axis=0)
            screen_array[xextra, yextra] = color_repete
...
particles.size_change(10)
particles.firework_size_change(10)
        ...
        for button in input_buttons:  # this is handler for buttons for operating eavents of buttons
            ...
            if button_action:
                if button_action in ["dust", "fireworks", "snow"]:
                    ...
                    particles.size_change(particle_info['amount'])
                    particles.firework_size_change(particle_info['amount'])
